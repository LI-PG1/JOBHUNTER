...
class JDAnalyzer:
    ...
    async def analyze(
        ...
        规则（2026-08-17）：岗位名称必填；JD 原文可缺省。
        无 JD 原文时降级：行业规则按岗位名匹配，LLM 按岗位名 + 用户技能推断方向/核心技能。
        """
        if not jobs:
            raise AppError(E_PARAM, "请至少填写 1 套目标岗位 JD（岗位名称必填）")
        blank_titles = [i + 1 for i, j in enumerate(jobs) if not j.title.strip()]
        if blank_titles:
            raise AppError(E_PARAM, f"岗位名称不能为空（第 {blank_titles[0]} 套）")
    ...
    def _match_industry(self, jobs: List[Job]) -> Optional[dict]:
        ...
        无 JD 原文时仅用岗位名称匹配关键词（降级，不阻塞）。
        """
        jd_text = " ".join(f"{j.title} {(j.jd_text or '').strip()}" for j in jobs).lower()
