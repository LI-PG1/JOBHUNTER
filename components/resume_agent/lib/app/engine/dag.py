...
运行流：
  pending → analyzing（JD 分析缓存复用 + 联网搜索 + 共享事实表）
         ...
         → reviewing（规则审核 → blocker 带意见重写 → 复审，≤MAX_REWRITE_ROUNDS）
...
STAGES = ["analyzing", "generating", "reviewing", "building"]
...
class GenerationRunner:
    ...
    async def run(self, task_id: str) -> None:
        ...
        try:
            ...
            if self._canceled(task_id):
                return
            await self._review(ctx)                # reviewing（规则审核 + blocker 重写）
            if self._canceled(task_id):
                return
    ...
    async def _review(self, ctx: GenContext) -> None:
        """规则审核回路（改造设计 §2/§3）：blocker 触发带意见重写，≤MAX_REWRITE_ROUNDS。"""
        from .review import run_review
        await run_review(self, ctx)

    # ------------------------------------------------------------ 阶段 4：building

    async def _build(self, ctx: GenContext) -> None:
        await self._set_stage(ctx.task_id, "building", 3)
        blocks = ctx.blocks
    ...
    async def _finish(self, ctx: GenContext) -> None:
        ...
        await self._push(ctx.task_id, "task.done", {
            ...
            **({"review": ctx.review_summary} if ctx.review_summary else {}),
        })
