...
async def gen_skill_extend(ctx: GenContext, *, review_feedback: Optional[str] = None) -> dict:
    if not ctx.skill_extend_enabled:
        return {"skills": [], "skipped": True}
    parsed = await llm_with_degrade(
        ctx.provider,
        skill_extend_messages(ctx.resume.get("skill") or [], ctx.jobs, review_feedback=review_feedback),
