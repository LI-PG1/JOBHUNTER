...
def jd_analysis_messages(jobs: List[dict], rules: dict, factsheet_input: dict) -> List[dict]:
    ...
    system = SYSTEM_PERSONA + (
        "\n\n你是 JD 分析器：从 1~5 套目标岗位中提炼职业方向与简历生成所需的关键事实，"
        "输出严格 JSON，不要输出任何解释或 markdown。"
        "若某岗位只有岗位名称没有 JD 原文，则基于岗位名称与用户技能合理推断核心技能，"
        "并在 jdFocus 中注明『岗位名称推断』。"
    )
    user = f"""请分析以下目标岗位（{len(jobs)} 套，属同一职业方向），输出共享事实表 JSON：
...
【目标岗位】
{"\n---\n".join(f"岗位：{j['title']}\nJD：{(j.get('jdText') or '').strip() or '（无 JD 原文，按岗位名称推断）'}" for j in jobs)}
...
【目标岗位 JD】
{"\n---\n".join(f"岗位：{j.get('title','')}\nJD：{(j.get('jdText') or '').strip() or '（无 JD 原文）'}" for j in jobs)}
...
def internship_messages(internships: List[dict], rules: dict, review_feedback: Optional[str] = None) -> List[dict]:
    ...
    3~4 条主题化条目（一页版由裁剪层裁至 3、两页版 4），每条「主题：内容描述（技术栈 + 量化成果）」
    （内容量固化 2026-08-20：单条等效字 ≥45，确保渲染 2~4 行、两页填充率 ≥85%）。
    """
    system = SYSTEM_PERSONA + (
        ...
        "duties=主要工作内容 3~4 条，每条格式为「主题：内容描述（含技术方案与量化成果）」，例如："
        ...
        "确实无法量化的写明覆盖范围/规模/频次），按重要性排序，保留 3~4 条最有价值的主题；"
        "**单条等效字数 ≥45**（中文 1 字=1、英文/数字≈0.55），保证渲染 2~4 行、信息密度生产级（内容量固化 2026-08-20）。输出严格 JSON。"
    )
    user = f"""{_estimated_protocol()}
...
def projects_messages(
    ...
    system = SYSTEM_PERSONA + (
        ...
        "**单条等效字数 ≥35**（中文 1 字=1、英文/数字≈0.55），保证渲染 2~4 行、信息密度生产级（内容量固化 2026-08-20）。输出严格 JSON。"
