...
@dataclass
class ReviewCfg:
    """审核回路配置（改造设计 §4.2 / §3.2 ③）：开关、重写轮数、禁用词表。"""
    enabled: bool = True
    max_rewrite_rounds: int = 1          # 每板块最多重写轮数（0~2）
    forbidden_words: list = field(default_factory=list)   # 禁用词表（命中 → blocker）
...
def load_config() -> Config:
    ...
    cfg.review = ReviewCfg(
        enabled=_get(r, "enabled", cfg.review.enabled),
        max_rewrite_rounds=_get(r, "max_rewrite_rounds", cfg.review.max_rewrite_rounds),
        forbidden_words=_get(r, "forbidden_words", cfg.review.forbidden_words) or [],
    )
    return cfg
